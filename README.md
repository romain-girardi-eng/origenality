...
**Site (v0).** The explorer joins eight authority-feed labels. Its 2 582 source
records are merged into 2 490 work clusters (92 duplicates collapsed):
<!-- FIGURES:population-root -->**2 490 records, of which 2 209 are counted**, 280 mentioned only, 1 held outside the count.<!-- /FIGURES:population-root -->
...
- **One harvest, one date.** <!-- FIGURES:harvest-dates -->The figures above describe the August 2026 harvest. The subject headings and containers of 1 173 of the 2 582 source records of the public site, from seven of the source labels, were read again from their own catalogues on 13 September 2026.
  There is no incremental update yet.<!-- /FIGURES:harvest-dates -->
- The public site is a deduplicated authority-record snapshot under eight source
  labels. The larger working corpus uses ten broader harvests and remains a
  separate population; figures never move between the two without being rebuilt.
...
Before a release, two checks read what no test can: whether the record links still
open a record on each source's own service, and whether every summary in the dump
resolves to its attribution. The first sends a few requests per source, at most one
per second per host, and is left out of the continuous integration for that reason.

```bash
python3 scripts/check_record_links.py --sample 5          # record links, per source (network)
python3 scripts/check_release.py <dump.jsonl>             # attribution of every summary
```
...
`api-spec/openapi.yaml` describes the JSON the command line prints, command by
command. No HTTP service implements it: the pages read the data files directly.

## Asking it from a program

The map answers one question, whether this ground has been worked and how
thickly, and the command line answers it the same way the website does, because both run
...
node cli/origenality.mjs claims Scarponi                # source-anchored positions
```
...
```
author:crouzel   year:1971-1990   lang:fr    type:book
...
`claims` reads the separate evidence projection. A claim carries its scholar,
scope, printed page, page hash and text position. Verbatim text is present only
when its release basis has been checked; otherwise the public quote is withheld
while the complete source anchor remains in the private research layer.
...
`widened` is true the query was relaxed to fewer terms, and the answer to the
